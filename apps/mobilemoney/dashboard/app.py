...
class MobileMoneyDashboardApplication(Application):
    name = None
    ...
    def get_urls(self):
        urlpatterns = [
            url(r'^transactions/$', self.list_view.as_view(),
                name='mobilemoney-transaction-list'),
            url(r'^transactions/(?P<pk>\d+)/$', self.detail_view.as_view(),
                name='mobilemoney-transaction-detail'),
        ]
...
application = MobileMoneyDashboardApplication()